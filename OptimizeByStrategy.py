...
def optimize_by_strategy(total_num_features, sampling_strategy="none", total_repeat_times=10, max_gen_num=10,
                         min_num_features=4):
    """
    ...
    filename = "./Experiment_result/F1-Score Error ComparisonGroup " + str(sampling_strategy) + ".csv"
    if os.path.exists(filename):
        logging.info(
            f"The file '{filename}' already exists, no need to run optimization with current sampling strategy!")
        return [], [], []
    ...
    f1_score_error_list, best_solution, best_recall = repeat_evolution(total_repeat=total_repeat_times,
                                                                       max_gen=max_gen_num,
    ...
    # save F1_score error, the best solution, best F1 score as csv file
    f1score_df = pd.DataFrame({"F1_Score Error": f1_score_error_list})
    f1score_df.to_csv("./Experiment_result/F1-Score ComparisonGroup " + str(sampling_strategy) + ".csv", index=False)

    best_solution = pd.DataFrame([best_solution])
    best_solution.to_csv("./Experiment_result/BestSolution " + str(sampling_strategy) + ".csv", index=False,
                         header=False)
